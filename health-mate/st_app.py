...
MODEL_PATH = 'leonfrank/InternLM2-PsyTherapist'


def run_api_server(path, model_format = 'awq', server_name = '0.0.0.0', server_port=23333, tp=1):
    local_path = snapshot_download(path)
    client = serve(local_path, server_name = server_name, server_port = server_port, tp = tp, model_format = model_format)
    if client:
        return
...
class SessionState:
    ...
    def init_state(self):
        ...
        action_list = [
            ArxivSearch(), PythonInterpreter(),  FinishAction()
        ]
...
class StreamlitUI:
    ...
    def setup_sidebar(self):
        ...
        model_ip = st.sidebar.text_input('模型IP：', value='0.0.0.0:23333')
        ...
        email_prompt = st.sidebar.text_area('邮件地址', value='someone@example.com')
        send_email = st.sidebar.button('发送对话邮件')
        convert_to_voice = st.sidebar.button('转化成语音播放')
    ...
    def init_model(self, option, ip=None):
        ...
        st.session_state['model_map'][option] = LMDeployClient(
            '/root/psy/internlm2-7b/work_dirs/internlm2_chat_7b_qlora_oasst1_512_e3_copy/hf_2/workspace_w4a16_kvcache',
            url=model_url,
            ...
            stop_words=['<|im_end|>'])
        st.session_state['model_map'][option].model_name = 'internlm2-chat-7b'
